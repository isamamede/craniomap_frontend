...
export default class ProfilePredictionsService {
  /**
  ...
  async writeForExistingParticipant(
    ...
    data: Omit<TProfilePredictions, "_id" | "participant_id" | "created_at">
  ) {
  ...
  async readByParticipant(
    participant_id: string
  ): Promise<Realm.Results<TProfilePredictions & Realm.Object>> {
    const db = await getRealm();
    try {
      const allPreds = db.objects<TProfilePredictions>(tablesNames.profilePred);
  ...
  async readById(id: string) {
    const db = await getRealm();
    try {
      const object = db.objectForPrimaryKey<TProfilePredictions>(
        tablesNames.profilePred,
        id
      );
      return object;
  ...
  async updateById(
    _id: string,
    data: Partial<Omit<TProfilePredictions, "_id">>
  ) {
    const db = await getRealm();
    try {
      return db.write(() => {
        ...
        if (object) {
          return db.create<TProfilePredictions>(
            tablesNames.profilePred,
            { _id, ...data },
  ...
  async deleteOne(_id: "string") {
    const db = await getRealm();
    try {
      var object = db.objectForPrimaryKey<TProfilePredictions>(
        tablesNames.profilePred,
        _id
      );
      db.write(() => {
